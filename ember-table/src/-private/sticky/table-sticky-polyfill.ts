...
class TableStickyPolyfill {
    ...
    rowMutationObservers?: MutationObserver[];
    resizeSensors?: any[];
   ...
   */
    repositionStickyElements = () => {
        const table = this.element.parentNode as HTMLTableElement;
        const scale = table.offsetHeight / table.getBoundingClientRect().height;
        const containerHeight = (table?.parentNode as HTMLElement)?.offsetHeight;
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            for (const child of row.children) {
                (child as HTMLElement).style.position = '-webkit-sticky';
                (child as HTMLElement).style.position = 'sticky';
                (child as HTMLElement).style[this.side as any] = `${offset}px`;
            }
...
export function setupTableStickyPolyfill(element: HTMLElement, offset = 0) {
