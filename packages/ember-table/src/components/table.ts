...
export default class TableComponent<R, F, TM> extends Component<TableArgs<R, F, TM>> {
    //ember-table's resizing must be enabled in order for fill-mode auto column
    //resizing to work, even if you don't want to allow user-invoked resizing
    ...
    get fillMode() {
        return this.args.fillMode ?? FillMode.FIRST;
    }
    ...
    get hasMoreRows() {
        return this.args.hasMoreRows ?? false;
    }
    ...
    get resizeMode() {
        return this.args.resizeMode ?? ResizeMode.STANDARD;
    }

    get resizeWidthSensitive() {
        return this.args.resizeWidthSensitive ?? true;
    }

    get rowComponent() {
        return this.args.rowComponent ?? 'ember-tr';
    }
    ...
    get headerStickyOffset() {
        return this.args.headerStickyOffset ?? 0;
    }

    get footerStickyOffset() {
        return this.args.footerStickyOffset ?? 0;
    }
    get sorts() {
        return this.args.sorts ?? [];
    }
