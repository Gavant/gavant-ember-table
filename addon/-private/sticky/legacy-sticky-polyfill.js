...
/**
  ...
  @see https://github.com/Addepar/ember-table/blob/v2.2.3/addon/-private/sticky/legacy-sticky-polyfill.js
*/

let LEGACY_POLYFILL_MAP = new WeakMap();

class LegacyStickyPolyfill {
    constructor(element, theadOffset, tfootOffset) {
        this.element = element;
        this.theadOffset = theadOffset;
        this.tfootOffset = tfootOffset;
    ...
    positionStickyElements = () => {
        ...
        if (thead) {
            ...
            thead.style.transform = `translateY(${scrollTop + this.theadOffset}px)`;
            this.fakeHeaderRow.style.height = `${thead.offsetHeight + this.theadOffset}px`;
        }

        if (tfoot) {
            tfoot.style.position = 'absolute';
            tfoot.style.transform = `translateY(${scrollTop + this.tfootOffset}px)`;
            this.fakeFooterRow.style.height = `${tfoot.offsetHeight + this.tfootOffset}px`;
        }
...
export function setupLegacyStickyPolyfill(element, theadOffset = 0, tfootOffset = 0) {
    LEGACY_POLYFILL_MAP.set(element, new LegacyStickyPolyfill(element, theadOffset, tfootOffset));
}
